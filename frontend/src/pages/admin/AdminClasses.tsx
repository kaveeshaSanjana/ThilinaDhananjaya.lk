...
export default function AdminClasses() {
  ...
  const classColumns: readonly StickyColumn<any>[] = [
    ...
    { id: 'monthlyFee', label: 'Monthly Fee', minWidth: 140, align: 'right', render: (cls) => <span className="text-slate-700 font-semibold text-sm">{cls.monthlyFee != null ? `Rs. ${Number(cls.monthlyFee).toLocaleString()}` : '-'}</span> },
    {
      id: 'status',
      label: 'Status',
      minWidth: 130,
    ...
    },
    {
      ...
      render: (cls) => (
        <div className="flex items-center justify-end gap-1.5">
          <Link to={`/admin/classes/${cls.id}`} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-xs font-semibold hover:bg-emerald-100 transition">
